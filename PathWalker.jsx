...
// Changes vs 1.0.3:
//   • Removed debug alert from loop generation
//   • Cycle group uses visible instead of enabled
//   • Convert to Bezier now preserves selected path index
//   • Star Type comparison hardened (>= 2)
//   • Refresh button uses text instead of Unicode arrow
//   • UI strings translated to Russian
//   • Help updated with Dot marker description
// Changes vs 1.0.4:
//   • Fixed: path vertices now baked through Vector Group transforms
//     (Illustrator-imported paths no longer jump off-place)
//   • Layer.parent copied to output layer
...
(function (thisObj) {
    ...
    function getSelLayer() {
        ...
        if (!sel || sel.length === 0) { alert("Выдели слой с маской или Shape-путём"); return null; }
        return sel[0];
    }
    ...
    // ============================================================
                ...
                for (var i = 1; i <= maskGrp.numProperties; i++) {
                    (function(idx){
                        ...
                        paths.push({
                            ...
                            transforms: []  // masks are in layer space
                        });
                    })(i);
                }
            }
        ...
        if (layer instanceof ShapeLayer) {
            try {
                ...
                function walk(group, breadcrumb, transforms) {
                    for (var i = 1; i <= group.numProperties; i++) {
                        var p = group.property(i);
                        if (!p) continue;
                        if (p.matchName === "ADBE Vector Group") {
                            var inner = p.property("ADBE Vectors Group");
                            var tr = readVectorGroupTransform(p);
                            if (inner) walk(inner, breadcrumb + "/" + p.name, transforms.concat([tr]));
                        } else if (p.matchName === "ADBE Vector Shape - Group") {
                            (function(pp, label, trs){
                                paths.push({
                                    kind: "shape",
                                    label: "Shape: " + label,
                            ...
                            })(p, breadcrumb + "/" + p.name, transforms);
                        }
                    }
                }
                if (root) walk(root, "", []);
    ...
    // Read Vector Group's transform: {anchor, position, scale, rotation, skew, skewAxis}
    function readVectorGroupTransform(vecGroup) {
        var tr = { anchor:[0,0], position:[0,0], scale:[100,100], rotation:0 };
        try {
            var g = vecGroup.property("ADBE Vector Transform Group");
            if (!g) return tr;
            try { tr.anchor   = g.property("ADBE Vector Anchor").value; }   catch(e){}
            try { tr.position = g.property("ADBE Vector Position").value; } catch(e){}
            try { tr.scale    = g.property("ADBE Vector Scale").value; }    catch(e){}
            try { tr.rotation = g.property("ADBE Vector Rotation").value; } catch(e){}
        } catch(e){}
        return tr;
    }

    // Apply a stack of Vector Group transforms to a point (innermost first? outermost first?).
    // AE composes transforms outer→inner, so we apply them from OUTERMOST (index 0) to INNERMOST.
    // Actually the geometry inside a group is transformed by that group.
    // So starting from the point in local (deepest) space, we apply innermost transform FIRST, then next up.
    // But since walk() pushes outermost first, then deeper, we iterate REVERSE.
    function applyVectorGroupStack(pt, stack) {
        var x = pt[0], y = pt[1];
        for (var i = stack.length - 1; i >= 0; i--) {
            ...
            y *= tr.scale[1] / 100;
            // 3) rotate
            var ang = tr.rotation * Math.PI / 180;
    ...
    // Apply Vector Group stack to a tangent vector (no translation, no anchor)
    function applyVectorGroupStackTangent(tan, stack) {
        var x = tan[0], y = tan[1];
        for (var i = stack.length - 1; i >= 0; i--) {
            var tr = stack[i];
            x *= tr.scale[0] / 100;
    ...
    function convertParametricToBezier(layer) {
        if (!(layer instanceof ShapeLayer)) {
            alert("Выдели Shape Layer для конверсии параметрических путей.");
    ...
    // ============================================================
        ...
        var stack = pathRef.transforms || [];
        var verts = [], inT = [], outT = [];
        for (var i = 0; i < p.vertices.length; i++) {
            verts.push(applyVectorGroupStack(p.vertices[i], stack));
            inT.push(applyVectorGroupStackTangent(p.inTangents[i], stack));
            outT.push(applyVectorGroupStackTangent(p.outTangents[i], stack));
        }
        return { verts: verts, inT: inT, outT: outT, closed: p.closed };
    }
...
function signedArea(verts) {
    ...
    function generate(opts) {
        ...
        if (!pathRef) { alert("Путь не выбран"); return; }

        var pd = readPathData(pathRef);
        if (!pd.verts || pd.verts.length < 2) { alert("У пути слишком мало вершин"); return; }
        ...
        if (numMarkers < 1) {
            alert("После кластеризации не осталось маркеров.\nУменьши 'Min vert dist'.\n\n" + filterDbg.join("\n"));
            return;
        }
        ...
        outLayer.moveBefore(srcLayer);
                ...
                // Copy full layer transform so path (baked in layer space) lands at the exact spot
        try {
            ...
            // Parent to same parent as source (preserves any hierarchy)
            try { if (srcLayer.parent) outLayer.parent = srcLayer.parent; } catch(e){}
        } catch(e) {}
    ...
    function buildUI(thisObj) {
        ...
        pathDD.selection = 0;
        var refreshBtn = rowPath.add("button", undefined, "Refresh");
        ...
        function refreshPaths() {
            ...
            if (pathsCache.length === 0) {
                pathDD.add("item", "(пути не найдены)");
                state.pathRef = null;
            } else {
        ...
        convBtn.helpTip = "Конвертирует параметрические Rect/Ellipse/Star в редактируемые Bezier Paths.";
            ...
            if (!lyr) { alert("Выдели Shape Layer."); return; }
            var savedIdx = pathDD.selection ? pathDD.selection.index : 0;
            app.beginUndoGroup(SCRIPT_NAME + ": Convert to Bezier");
            var n = convertParametricToBezier(lyr);
            app.endUndoGroup();
            alert("Конвертировано путей: " + n);
            refreshPaths();
            if (pathDD.items.length > savedIdx) pathDD.selection = savedIdx;
        };
        ...
        mergeDD.helpTip = "Centroid: маркер в центре кластера, прямые линии между ними.\nSmooth: маркер на пути, сегменты повторяют кривые.";
        ...
        btnGo.onClick = function(){
            if (!state.pathRef) { alert("Путь не выбран. Нажми Refresh, выделив слой с маской или Shape-путём."); return; }
            if (!state.showMarkers && !state.showTrace) { alert("Включи хотя бы 'Show markers' или 'Show trace'."); return; }
...
"  Dot и остальные — с заливкой; Circle — кольцо (только stroke).\n" +
