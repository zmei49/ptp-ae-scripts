...

{
    ...
    function buildUI(thisObj) {
        ...
        styleLabel(lblOv);
        var slOver = grpOver.add("slider", undefined, 120, 70, 200);
        ...
        slOver.onChanging = function() { etOver.text = Math.round(slOver.value).toString(); };
            ...
            if (!isNaN(v) && v >= 70 && v <= 200) slOver.value = v;
    ...
    function getActiveComp() {
       ...
       function getSelectedLayer(comp) {
        ...
        if (!sel || sel.length === 0) {
            alert("Выдели хотя бы один слой в композиции.");
            return null;
        }
        return sel[0];
    }
    ...
    function svec(layer, pct) {
        ...
        function getPositionPropSilent(layer) {
        try {
            var pos = layer.property("ADBE Transform Group").property("ADBE Position");
            if (pos && pos.numKeys >= 2) return pos;
        } catch(e){}
        return null;
    }
    ...
    // ====================================================================
        ...
        try {
            ...
            for (var li = 0; li < layers.length; li++) {
                var layer = layers[li];
                var pos = getPositionPropSilent(layer);
                if (!pos) continue;
                ...
                for (var r = 0; r < repeat; r++) {
                    if (isIn) {
                        ...
                        // Opacity: 0 → 100 только за первый повтор, дальше держим 100
                        if (r === 0) {
                        ...
                        if (r === repeat - 1) {
                            ...
                            // Opacity: 100 весь путь, 0 в самом конце
                            opacity.setValueAtTime(s0, 100);
                            opacity.setValueAtTime(s1, 0);
                        } else {
                            scale.setValueAtTime(s1, svec(layer, 100));
                            opacity.setValueAtTime(s0, 100);
                            opacity.setValueAtTime(s1, 100);
                        }
                    }
                }
       ...
       function runZoom(isIn, duration) {
        ...
        } catch (e) { alert("Drop-in error: " + e.toString()); }
        app.endUndoGroup();
    }
    ...
    function runWaypointPulse(duration, overshootPct) {
        ...
        try {
            ...
            for (var li = 0; li < layers.length; li++) {
                ...
                if (!rot) continue;
                var firstT = pos.keyTime(1);
                var lastT  = pos.keyTime(pos.numKeys);
                var totalDur = lastT - firstT;

                // Адаптивный step: минимум 0.2 сек, максимум 20 keyframes
                var step = Math.max(0.2, totalDur / 20);
                var amp = 5, sign = 1;

                for (var t = firstT; t <= lastT; t += step) {
                    rot.setValueAtTime(t, sign * amp);
                    sign *= -1;
                }
                applyEaseToAllKeys(rot, "Sine", "InOut");
            }
