...
// Changes vs v1.0:
//   • titleH now scales with fontSize (was hard-coded 22)
//   • "!" bang in banner now has proper Anchor Point
//   • Popup_N counter avoids overwriting existing pre-comps
//   • Font fallback: Consolas → Menlo → Courier New
//   • New option: "Unique data per copy"
//   • Rounded stroke widths (no half-pixel bleed)
//   • Minimum stagger 0.05 s (prevents cascade collapse)
...
(function ptp_PopupCascade(thisObj) {
    ...
    function randomTitleId(themeMode) {
        var prefix;
        if (themeMode === "system") {
            var arrSys = ["SYS","CORE","UNIT","NODE","HUB","CTL"];
            prefix = arrSys[Math.floor(Math.random()*arrSys.length)];
        } else if (themeMode === "gps") {
            var arrGps  = ["GPS_TRACKER","ROUTE_LOG","NAV_DATA","GEO_FIX","WAYPOINT","TELEMETRY"];
            return arrGps [Math.floor(Math.random()*arrGps.length)] + "_" + Math.floor(Math.random()*99);
        } else {
            prefix = "SYS";
        }
    ...
    function buildWindow(parentComp, opts, idx) {
        ...
        if (opts.showIcon) {
            ...
            try {
                ...
                // Ставим по baseline: центр круга по Y = yCursor+9, baseline "!" должен быть на ~yCursor+13
                // (высота "!" в 14pt ≈ 10px, baseline ниже верха на ~10, значит center = baseline - 5)
                bangLayer.property("Transform").property("Position").setValue([padding + 14, yCursor + 13]);
                // Anchor не трогаем (оставим дефолт), центрирование делается через justification=CENTER
            } catch(e){}
       ...
       function buildStripes(layer, w, h, color) {
        ...
        var step = 6;
        // Линии идут диагонально от (x, 0) до (x-h, h).
        // Обрезаем оба конца по прямоугольнику [0..w] × [0..h].
        for (var x = -h; x <= w + h; x += step) {
            var x1 = x,     y1 = 0;
            var x2 = x - h, y2 = h;

            // Обрезка по X=0 (левая граница)
            if (x1 < 0 && x2 > 0) {
                var t = (0 - x1) / (x2 - x1);
                x1 = 0; y1 = t * h;
            } else if (x2 < 0 && x1 > 0) {
            ...
            // Пропустить если обе точки за пределами (одинаковой стороны)
            if ((x1 < 0 && x2 < 0) || (x1 > w && x2 > w)) continue;

            addLinePath(inner, x1, y1, x2, y2);
        }
        addStroke(inner, color, 2, 100);
        try {
    ...
    function buildStripesInComp(compOb, w, h, opts) {
    var shapeLyr = compOb.layers.addShape();
    shapeLyr.name = "Stripes";
    var contents = shapeLyr.property("Contents");

    // фон
    var bgGrp = contents.addProperty("ADBE Vector Group");
    var bgInner = bgGrp.property("ADBE Vectors Group");
    var bgPath = bgInner.addProperty("ADBE Vector Shape - Rect");
    try { bgPath.property("ADBE Vector Rect Size").setValue([w, h]); } catch(e){}
    var bgFill = bgInner.addProperty("ADBE Vector Graphic - Fill");
    try { bgFill.property("Color").setValue(opts.bannerColor); } catch(e){}

    // якорь в центр композиции — вершины считаем от левого-верхнего угла (-w/2, -h/2)
    var stripeGrp = contents.addProperty("ADBE Vector Group");
    var inner = stripeGrp.property("ADBE Vectors Group");
    var step = Math.max(4, Math.round(h * 0.5));
    for (var x = -h; x <= w + h; x += step) {
        var sh = inner.addProperty("ADBE Vector Shape - Group");
        var s = new Shape();
        s.vertices = [[x - w/2, -h/2], [x - w/2 - h, h/2]];
        s.inTangents = [[0,0],[0,0]];
        s.outTangents = [[0,0],[0,0]];
        s.closed = false;
        try { sh.property("ADBE Vector Shape").setValue(s); } catch(e){}
    }
    var stroke = inner.addProperty("ADBE Vector Graphic - Stroke");
    try { stroke.property("Color").setValue([1,1,1]); } catch(e){}
    try { stroke.property("Stroke Width").setValue(Math.max(1, Math.round(h * 0.06))); } catch(e){}
}
    ...
    function generateCascade(opts) {
        ...
        var idx = 1;
while (findItemByName("Popup_" + idx)) idx++;
var basePrecomp = buildWindow(comp, opts, idx);


        // 2. Создаём родительский Null для удобного перетаскивания каскада
        var nullLayer = comp.layers.addNull();
        nullLayer.name = "PopupCascade_CTRL_" + pad3(idx);
        try {
            nullLayer.property("Transform").property("Position").setValue([comp.width/2, comp.height/2]);
        } catch(e){}

        // 3. Размещаем N копий, привязываем к Null
        var stagger = Math.max(0.05, opts.spawnDuration / Math.max(1, opts.copiesCount));
        var placed = [];
    ...
    var srcPrecomp = basePrecomp;
    if (opts.uniquePerCopy && i > 0) {
        var idx2 = idx + i;
        while (findItemByName("Popup_" + idx2)) idx2++;
        srcPrecomp = buildWindow(comp, opts, idx2);
    }
    var layer = placeCopy(comp, srcPrecomp, i, opts.copiesCount, opts, spawnTime, cycleTotal);
    ...
    function buildUI(thisObj) {
        ...
        var cbUnique = w.add("checkbox", undefined, "Unique data per copy");
cbUnique.value = false;
cbUnique.helpTip = "Каждая копия будет использовать свой pre-comp с собственными GPS-данными. Больше памяти.";
        loopCB.value = true;
    ...
    function getHelpText() {
        return SCRIPT_NAME + " " + SCRIPT_VERSION + "\n" +
            ...
            "  пересоздай каскад.\n\n" +
            "═══ ОПЦИИ v1.1 ═══\n" +
            "• Unique data per copy — каждая копия получает свой pre-comp\n" +
            "  с собственными случайными GPS-данными (больше памяти).\n\n" +
